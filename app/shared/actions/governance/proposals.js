...
const defaultContract = 'enuforumrcpp';

export function getProposals(scope = 'enuforumdapp', previous = false) {
...
export function getVoteInfo(scope, account) {
  return (dispatch: () => void, getState) => {
    ...
    const formatted = enu(connection).modules.format.encodeNameHex(account, true)
