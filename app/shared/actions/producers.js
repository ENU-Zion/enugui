...
export function getProducers(previous = false) {
  return (dispatch: () => void, getState) => {
    ...
    enu(connection).getTableRows(query).then((results) => {
      ...
      if (contract && contract['enu.token'] && contract['enu.token'][connection.chainSymbol]) {
        const supply = parseFloat(contract['enu.token'][connection.chainSymbol].supply);
        // yearly inflation
      ...
      const data = rows
        .filter((p) => (p.producer_key !== 'ENU1111111111111111111111111111111114T1Anm'))
        .map((producer) => {
