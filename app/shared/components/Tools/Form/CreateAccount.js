...
class ToolsFormCreateAccount extends Component<Props> {
  constructor(props) {
    ...
    this.state = {
      ...
      delegatedBw,
      delegatedCpu,
      ENUbalance: (balance && balance.ENU) ? balance.ENU : 0,
      formErrors: {},
      ownerKey,
      ramAmount,
      submitDisabled: true
  ...
  errorsInForm = (errors, disabled) => {
    ...
    const decimalBalance = Decimal(ENUbalance);
              ...
              >
                ...
                {(ramPrice && !formErrors.ramAmount) ? (
                  <h4 style={{ margin: '30px' }}>
                    {`${t('tools_form_create_account_ram_price_estimate')} ${ramPrice.toFixed(4)} ENU.`}
                  </h4>
                ) : ''}
                <FormMessageError
                  errors={
