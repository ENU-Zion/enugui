...
class DevTest extends Component<Props> {
  ...
  testStake = () => {
    ...
    actions.delegatebw(wallet.account, 'jestaaaaaaaa', '0.0000 ENU', '0.0001 ENU');
    this.setState({ actionName: 'DELEGATEBW' });
  }
  testTransfer = () => {
    const { actions, wallet } = this.props;
    actions.transfer(wallet.account, 'jestaaaaaaaa', '0.0001 ENU', '');
    ...
    return (
      <Grid>
        <Grid.Column width={4}>
          ...
          >
            <Button
              content="Transfer 0.0001 ENU"
              onClick={this.testTransfer}
              primary
            />
            <Button
              content="Stake 0.0001 ENU"
