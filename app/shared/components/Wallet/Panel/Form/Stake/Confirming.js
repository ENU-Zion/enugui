...
class WalletPanelFormStakeConfirming extends Component<Props> {
  ...
  render() {
    ...
    const enuAccount = new ENUAccount(account, balance);

    const refundDate = enuAccount.getRefundDate();
        ...
        <Segment padding="true" basic>
          ...
          <Segment.Group>
            {(netDifference > 0) ? (
              <Segment>
                <Header textAlign="center">
                  <font color="green">
                    <Icon name="wifi" />{t('about_to_stake_units_to_net', { amount: `${netDifference.toFixed(4)} ${connection.chainSymbol || 'ENU'}` })}
                  </font>
                  <Header.Subheader>
            ...
            {(netDifference < 0) ? (
              <Segment>
                <Header textAlign="center">
                  <font color="red">
                    <Icon name="wifi" />{t('about_to_unstake_units_from_net', { amount: `${(-netDifference).toFixed(4)} ${connection.chainSymbol || 'ENU'}` })})
                  </font>
                  <Header.Subheader>
                    ({t('will_have_units_in_net_after', { amount: `${netAmount.toFixed(4)} ${connection.chainSymbol}` })})
                  </Header.Subheader>
            ...
            {(cpuDifference > 0) ? (
              <Segment>
                <Header textAlign="center">
                  <font color="green">
                    <Icon name="microchip" />{t('about_to_stake_units_to_cpu', { amount: `${(cpuDifference).toFixed(4)} ${connection.chainSymbol || 'ENU'}` })}
                  </font>
                  <Header.Subheader>
            ...
            {(cpuDifference < 0) ? (
              <Segment>
                <Header textAlign="center">
                  <font color="red">
                    <Icon name="microchip" />{t('about_to_unstake_units_from_cpu', { amount: `${(-cpuDifference).toFixed(4)} ${connection.chainSymbol || 'ENU'}` })}
                  </font>
                  <Header.Subheader>
                    ({t('will_have_units_in_cpu_after', { amount: `${cpuAmount.toFixed(4)} ${connection.chainSymbol}` })})
                  </Header.Subheader>
          ...
          {(unstakingWhenAmountBeingUnstaked) ? (
            <Message
              ...
              warning="true"
            >
              {t('have_already_unstaked')} {enuAccount.getTotalBeingUnstaked().toFixed(4)} ENU {t('unstaking_will_be_reset')}
            </Message>
          ) : ''}
