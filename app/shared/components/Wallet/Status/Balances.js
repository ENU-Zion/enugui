...
class WalletStatusBalances extends Component<Props> {
  render() {
    ...
    const totalTokens = totalStaked + totalBeingUnstaked + ((tokens.ENU) ? tokens.ENU : 0);
    const rows = [
      (
        <Table.Row key="ENU">
          <Table.Cell width={2}>
                ...
                {(refund_request)
                  ? (
                    <Table.Row>
                      ...
                      <Table.Cell>{totalBeingUnstaked.toFixed(4)} ENU (<TimeAgo date={refundDate} />)</Table.Cell>
                    </Table.Row>
                  )
                  : false
                }
