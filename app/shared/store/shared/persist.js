  ...
  */
  3: (state) => {
    ...
    if (
      ...
      newSettings.customTokens = ['enu.token:ENU'];
    }
    if (
      newSettings.customTokens
      && newSettings.customTokens.indexOf('enu.token:ENU') === -1
    ) {
      newSettings.customTokens.push('enu.token:ENU');
    }
  ...
  */
  7: (state) => {
    ...
    if (modifiedWallet.pubkey && modifiedWallet.accountData) {
      const auth =
        new ENUAccount(modifiedWallet.accountData)
    ...
    wallets.forEach((currentWallets) => {
      ...
      if (currentWallets.pubkey && currentWallets.accountData) {
        const auth =
          new ENUAccount(currentWallets.accountData)
